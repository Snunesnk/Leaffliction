...
std::vector<double> getTransformtionInfo(cv::Mat& image, const int transformation)
{
	if (transformation == 1) {
		...
		//// La moyenne des valeurs de pixels des canaux (rouge, vert, bleu) pour chaque bande de fr�quence de Fourier 
		...
		//	// S�parer les parties r�elles et imaginaires
		//	std::vector<cv::Mat> channels(2);
		//	cv::split(complexChannel, channels);
		//	cv::Mat realPart = channels[0];

		//	// Calculer la moyenne des valeurs de la partie r�elle pour chaque bande de fr�quence
